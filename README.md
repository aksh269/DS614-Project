...
### Step 1 — Entry Point: `ClientContext::Query()`

**File:** [`src/main/client_context.cpp`](https://github.com/duckdb/duckdb/blob/master/src/main/client_context.cpp)

`ClientContext` is DuckDB's session-level state holder — it owns the connection, transaction state, and configuration. Every SQL string enters here. The parsing, planning, and execution phases are **strictly separated**, which allows the optimizer to rewrite the logical plan before any physical decisions are made.

```cpp
// src/main/client_context.cpp (simplified)
unique_ptr ClientContext::Query(const string &query,
                                              bool allow_stream_result) {
    // Phase 1: Parse SQL → Logical AST (no execution)
    auto statements = ParseStatements(query);

    // Phase 2: Plan logical → physical (no execution yet)
    auto physical_plan = planner.CreatePlan(*statements[0]);

    // Phase 3: Execute physical plan
    return executor.Execute(physical_plan);
}
```

**Why this separation matters:** The optimizer sits between phases 1 and 2. It rewrites the logical plan (predicate pushdown, join reordering, projection pruning) before a single physical operator is chosen. This is why `SELECT * FROM lineitem WHERE 1=0` executes instantly — the optimizer eliminates it before any data is touched.

---

### Step 2 — Physical Plan: `PhysicalPlanGenerator::CreatePlan()`

**File:** [`src/execution/physical_plan_generator.cpp`](https://github.com/duckdb/duckdb/blob/master/src/execution/physical_plan_generator.cpp)

The logical plan is lowered to a physical execution DAG. For our query:
...
This is where DuckDB's **push model** is chosen over PostgreSQL's Volcano pull model:

```
Volcano (PostgreSQL) — PULL:          DuckDB — PUSH:
────────────────────────────          ─────────────────────────────
HashAggregate calls next() on child   Executor calls Execute() on each op
child returns ONE row                 Each op receives 2048-row DataChunk
repeat 100M times                     repeat 48,828 times
= 100M virtual function calls         = 48,828 function calls  (2048x fewer)
```

```cpp
// src/execution/physical_plan_generator.cpp (simplified)
unique_ptr
PhysicalPlanGenerator::CreatePlan(LogicalOperator &op) {
    switch (op.type) {
    case LogicalOperatorType::LOGICAL_GET:
        return CreatePlan((LogicalGet &)op);   // → PhysicalTableScan

    case LogicalOperatorType::LOGICAL_AGGREGATE_AND_GROUP_BY:
        return CreatePlan((LogicalAggregate &)op); // → PhysicalHashAggregate

    case LogicalOperatorType::LOGICAL_FILTER:
        return CreatePlan((LogicalFilter &)op);    // → PhysicalFilter
...
### Step 3 — Pipeline Scheduling: `Executor::ExecuteTask()`

**File:** [`src/execution/executor.cpp`](https://github.com/duckdb/duckdb/blob/master/src/execution/executor.cpp)

The physical plan is decomposed into **Pipeline** objects — linear chains of operators with no branching. Each pipeline is scheduled as an independent parallel task. Multiple CPU cores run different pipelines simultaneously, sharing only the final result buffer.

```cpp
// src/execution/executor.cpp (simplified)
void Executor::ExecuteTask(ExecutionContext &context, Pipeline &pipeline) {
    // Each task processes one DataChunk through the entire operator chain
    auto chunk = pipeline.source->GetData(context);    // TableScan reads 2048 rows
    
    for (auto &op : pipeline.operators) {
        op->Execute(context, *chunk, *next_chunk);     // Each op processes full chunk
        swap(chunk, next_chunk);
    }
    
    pipeline.sink->Sink(context, *chunk);  // HashAggregate absorbs final chunk
}
```

---

### Step 4 — Core Data Structure: `DataChunk`

**File:** [`src/common/types/data_chunk.cpp`](https://github.com/duckdb/duckdb/blob/master/src/common/types/data_chunk.cpp)

The `DataChunk` is DuckDB's fundamental unit of data movement. It holds up to `STANDARD_VECTOR_SIZE` (2048) rows with one `Vector` per column.

```cpp
// src/common/types/data_chunk.cpp
class DataChunk {
public:
    // One Vector per column — each Vector is a contiguous array
    vector data;
    
    // Number of rows currently stored (≤ STANDARD_VECTOR_SIZE = 2048)
    idx_t count;
    ...
    void Initialize(ClientContext &context,
        ...
        for (idx_t i = 0; i < types.size(); i++) {
            data[i].Initialize(context, types[i], capacity);
        }
    }
...
**Memory layout — why this enables cache efficiency:**

```
                    Addresses in RAM
                    ────────────────
l_quantity column:  0x1000 [17] [36] [12] [45] [23] [8] ... (2048 × 8 bytes = 16 KB)
l_returnflag col:   0x5000 [A]  [A]  [R]  [N]  [R]  ... (2048 × 1 byte  =  2 KB)

The CPU prefetcher sees a predictable stride:
  Access l_quantity[0] at 0x1000 → prefetch 0x1008, 0x1010, 0x1018 ...
  Every access is a cache hit. Miss ratio ≈ 5%.

Compare to row storage:
  Row 0 → address 0x1000 (all 50 columns)
  Row 1 → address 0x11A0 (next row, 416 bytes later)
  Prefetcher sees: 0x1000, 0x11A0, 0x1340 ... unpredictable stride
  Miss ratio ≈ 90%.
```

---

### Step 5 — Tight-Loop Operator: `PhysicalHashAggregate::Execute()`

**File:** [`src/execution/operator/aggregate/physical_hash_aggregate.cpp`](https://github.com/duckdb/duckdb/blob/master/src/execution/operator/aggregate/physical_hash_aggregate.cpp)

```cpp
// src/execution/operator/aggregate/physical_hash_aggregate.cpp (simplified)
SinkResultType PhysicalHashAggregate::Sink(ExecutionContext &context,
                                            DataChunk &input,
                                            OperatorSinkInput &input_state) {
    // input = DataChunk with 2048 rows
    auto &group_vectors  = input.data[0];  // l_returnflag — contiguous array
    auto &payload_vector = input.data[2];  // l_quantity   — contiguous array

    // THIS TIGHT LOOP is what the compiler vectorizes into AVX/SSE instructions
    // No pointer aliasing, no branches, sequential access → SIMD auto-emission
    for (idx_t i = 0; i < input.size(); i++) {
        // hash_table lookup + aggregate update
        // Compiler emits: VPADD ymm0, ymm1, ymm2  (adds 4 int64s in one instruction)
        ht->AddChunk(context.client, input);
    }
    return SinkResultType::NEED_MORE_INPUT;
}
```

**Why this loop gets SIMD from the compiler:**

```
Condition 1: No pointer aliasing
  group_vectors and payload_vector point to different memory regions
  Compiler can guarantee they don't overlap → safe to reorder loads

Condition 2: No loop-carried dependencies  
  Each iteration's result doesn't depend on the previous iteration's result
  Compiler can execute 4–8 iterations in parallel

Condition 3: Contiguous, stride-1 memory access
  payload_vector[i], payload_vector[i+1] are adjacent in memory
  CPU can load 4 values in one cache line fetch

Result: Compiler emits AVX2 instructions:
  vmovdqu   ymm0, [rsi + i*8]     ; load 4 × int64 from payload
  vpaddq    ymm1, ymm0, ymm2      ; add 4 values simultaneously  
  vmovdqu   [rdx + i*8], ymm1     ; store 4 results
```
...
```
main() / Python: duckdb.sql("SELECT ...")
  │
  ▼
ClientContext::Query()                              src/main/client_context.cpp
  │
  ├─ Parser::ParseQuery()                           src/parser/parser.cpp
  │    └─ Returns: list of Statement objects (AST)
  │
  ├─ Planner::CreatePlan()                          src/planner/planner.cpp
  │    ├─ Optimizer::Optimize()                     src/optimizer/optimizer.cpp
  │    └─ PhysicalPlanGenerator::CreatePlan()       src/execution/physical_plan_generator.cpp
  │         └─ Returns: PhysicalOperator tree (DAG)
  │
  └─ Executor::Execute()                            src/execution/executor.cpp
       ├─ PipelineBuilder::Build()                  src/execution/pipeline.cpp
       └─ For each Pipeline:
            ├─ PhysicalTableScan::GetData()         src/execution/operator/scan/
            │    └─ Returns: DataChunk (2048 rows)       physical_table_scan.cpp
            │
            └─ PhysicalHashAggregate::Sink()        src/execution/operator/aggregate/
                 └─ Updates in-memory hash table        physical_hash_aggregate.cpp
