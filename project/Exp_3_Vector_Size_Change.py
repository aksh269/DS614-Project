...
def build_duckdb():
    run_cmd(["cmake", "-B", "build", "-S", ".", "-DCMAKE_BUILD_TYPE=Release", "-DBUILD_EXTENSIONS=tpch"], cwd=DUCKDB_ROOT)
    run_cmd(["cmake", "--build", "build", "--config", "Release"], cwd=DUCKDB_ROOT)

    if not DUCKDB_BIN.exists():
        raise RuntimeError(f"duckdb executable not found after build at {DUCKDB_BIN}")
